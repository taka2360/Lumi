...
**その帰結として、購読者が「いま処理している stream」へ publish することを禁止する。**
検出したら `ReentrantPublishError` を投げる。**待たせない**（`asyncio.Lock` は再入不可なので、
放置すれば無言のデッドロックになる）。他の stream への publish は従来どおり自由である。
...
配送中にロックを保持すると、購読者が同じ stream へ publish した瞬間にデッドロックする。
選択肢は3つあった。

**再入を許すこと自体が矛盾している。** 購読者が `seq=1` を処理している最中に発行する
`seq=2` は、定義上 `seq=1` の処理の**後**に順序づけられなければならないが、
その処理はまだ終わっていない。**「同一 stream 内では処理も順序保証する」と両立しない。**
...
購読者が同じ stream へ publish できる。**既存コードを何も壊さない**のが利点。
...
- 購読者から同一 stream へ publish できなくなる（**Phase 1 に該当箇所は無い**）
...
2. `ReentrantPublishError` を追加する。購読者の中から同一 stream へ publish すると送出される
