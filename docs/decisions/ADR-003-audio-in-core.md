...
> **〔2026-08-18 追記〕 p50 は 1.5 s に改定された**（p95 / p99 は据え置き）。
> 実測前に置いた 1.2 s が、`vad_ms` の実測 0.43 s と両立しなかったため。
> **この ADR は決定時点の記録なので数値は書き換えない。**
> 現行の値の唯一の定義場所は [../architecture/audio.md](../architecture/audio.md) §7。
