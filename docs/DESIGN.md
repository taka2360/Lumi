...
>    4回別々に聞くと、**実際に決め手になる数字（全部でいくらか）だけが最後まで出てこない**
> 2. **埋め込みモデルは4つ目の component になったが、起動をブロックしない。**
>    ADR-034 の「3つ揃うまで出さない」の3つには入らない——
>    無くても会話は成立し、落ちるのは意味検索だけ
> 3. **パネルウィンドウは接続時にスナップショットを受け取る。** Core は変化したときに
>    配信するので、**あとから開いた窓は自分に関係する変化を全部逃している**
> 4. 記憶ウィンドウの「まだ何も覚えていません」を3つに割った——
>    **未接続 / 検索に一致なし / まだ読んでいない会話が N 行ある**
