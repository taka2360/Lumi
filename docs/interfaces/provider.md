...
- ★ **抽出の 1536 は暴走の保険であり、記憶層の受け皿とセットでのみ成立する**
  （[../decisions/ADR-049-truncated-extraction-and-watermark.md](../decisions/ADR-049-truncated-extraction-and-watermark.md)）。
  抽出は次のパスでも同じ Episode を読むので、**入力だけで決まる打ち切りはその入力に対して毎回再発する**——
  受け皿（バッチ縮小）が無いまま上限だけ入れると抽出キューが止まる。
  終わり方ごとの扱いは [../architecture/memory.md](../architecture/memory.md) §4 が単一定義
- **1536 は導出であって実測ではない。** `CANDIDATE_LIMIT = 8` を超えた要素はパーサが捨てるので、
  **読まれうる最大の答えは約 850 トークン**（日本語 `content` 1件で 90〜100）。その 1.8 倍を取った。
  抽出の出力トークン数を測ったら引き直す
