...
| 取るべき値 | 実測 |
|---|---|
| 投機の破棄率 | **12.5%**（実行 8 回に対し `stt.speculation_discarded` 1 回）。ターンで数えると 7 ターン中 1 ターン。**分母の数え方には注意が要る**（下記） |
...
**`stt.speculation_started` は要求の数であって、実行の数ではない。**
`speculate()` はワーカーが空いているかを見る**前に**これを出す（`core/lumi/agent/stt.py`）。
埋まっていれば要求は `_pending` に入り、**より新しい世代に上書きされるか、上限や
ターン境界で解放されると、一度も実行されないまま消える。**
消えたものは `_done` に到達しないので、**`stt.speculation_discarded` にも現れない。**

**つまり要求で割ると、混み合ったターンほど破棄率が低く出る**——
この指標が暴きたい状況で、まさに過小に出る。

**この 7 ターンでは両者が一致している。** 上書きが起きるのは
「前の実行が走っている最中に次の投機が要求されたとき」だが、
ログ上、後続の `stt.speculation_started` はいずれも直前の実行の所要時間より後に来ている
（唯一の同一ターン2回であるターン 4 も、gen 4 は 35.569 頃に完了しており、
gen 5 の要求 36.380 の時点でスロットは空いていた）。**8 要求はすべて実行された。**

**ただしこれは「この run ではたまたま一致した」であって、指標が正しいことの保証ではない。**
そこで**計測器の側を直した**——`_start()` で `stt.speculation_execution_started` を、
実行されずに消えた要求で `stt.speculation_dropped` を出すようにした。
**8g / 8h で取り直すときは、分母に `stt.speculation_execution_started` を使う。**
...
| （参考）`stt_ms` 自体 | 220 | 158 | **267** | 最大は目標超過。**隠れた根拠は「430 未満」ではなく、全ターンで `stt_overlap_ms == stt_ms` だったこと**（下記） |
...
| **混み合ったときの破棄率** | 7 ターンで**保留スロットの上書きが一度も起きていない。** 12.5% は**競合が無い場合の値**である |
