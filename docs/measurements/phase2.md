...
- **`<|ACT ...|>` は本番と同じ `MarkerStream` で取り除いてから数える。**
  `SPEECH_PROTOCOL` は毎ターン マーカーを要求しており、本番は
  [streaming.py](../../core/lumi/agent/streaming.py) で除去した text を記録・発話する。
  マーカーは ASCII で40字前後あり、30〜120字の日本語応答に残すと **字数 と 反復 が歪む**。
  しかも**マーカーの出る頻度はプロファイルごとに違う**ので、これは定数のずれではなく
  変種間の差になる。出た回数は `marker` 列に出す（sampling の結果ではなく、
  プロファイルが `SPEECH_PROTOCOL` にどれだけ従うかの観察として）
- **`Finish(reason="stop")` かつ、マーカーを除いた発話テキストが空でないものだけが
  sample である。** `length` / tool call / 空応答 / `LLMFailure` / `Finish` 無しは
  **除外**し、理由を印字する
- ★ **「`stop` したが喋っていない」は簡潔なのではなく無言である。** sample に数えると
  **その run で最短・最速の応答**として出て、そのプロファイルが見事に簡潔に見える。
  マーカーだけを出したターンもここに落ちる
...
- **変種ごとに、捨てるための生成を1回ずつ入れる**（ケースに含まれない中立の1ターン）。
  1回目のリクエストが system prompt の KV キャッシュを冷たいまま払う問題
  （644 ms 対 ~420 ms → [phase1.md](phase1.md)）は run 全体で1回でも消えるが、
  **`CASES[0]` を温めると、その直後に走る変種だけがユーザー発話ぶんまでキャッシュに載る。**
  差は数トークンぶんで 644/420 よりずっと小さいが、**毎回同じ変種の、印字される1セルに乗る。**
  変種ごとに中立のターンを回せば5つとも同じキャッシュ状態から始まる。
  latency は**変種間の比較にだけ**使える（SLO の測定は [phase1.md](phase1.md) の役目）
- **分類（どれを sample と数えるか）は LLM 無しでテストしてある** →
  `core/tests/test_scripts_profile_eval.py`。証拠として引く以上、
  ここが黙って壊れるのは数値が黙って壊れるのと同じである
