...
### なぜ完全に隠れたのか — 余裕は 134 ms しかない

隠れる条件は `stt_ms < vad_ms` の一点である。

| | |
|---|---|
| 無音待ち（`min_silence_duration_ms` + フレーム境界） | **401〜407 ms** |
| `stt_ms` の最大 | **267 ms** |
| **余裕** | **134 ms** |

**`large-v3-turbo` を CUDA で回して、なお 134 ms 残っている。**
phase1 の `small`（p50 68 ms）から**モデルを大きくしても隠れきった**のは、
無音待ちが 400 ms という長さだからである。

**この余裕は構成が変わると簡単に消える。** STT が 1.5 倍遅くなれば（400 ms）隠れなくなり、
