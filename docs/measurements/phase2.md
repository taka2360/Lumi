...
36.526  stt.speculation_discarded generation=4 144 ms  ← 走っていた 4 が完走し、照合で落ちた
36.774  reactive.stt  stt_ms=145 overlap_ms=145 discarded=1
```

| 観測 | 意味 |
|---|---|
| gen 5 の開始（36.380）より gen 4 の破棄（36.526）が**後** | 走行中の STT はキャンセルされていない。**保留スロットの上書き（latest-wins）が実機でそのとおり動いている** |
| 36.526 → 36.774 が 248 ms で、そのうち `stt_ms` が 145 ms | gen 4 の完走を待ってから gen 5 が走った。**同時に走った STT は最後まで1つ**（single-flight） |
...
| | |
|---|---|
| **CPU 構成** | 測っていない。audio.md §7 の 0.49 s は**依然として予測値である** |
