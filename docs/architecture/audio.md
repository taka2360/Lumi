...
| **合計** | | **1.27 s** | **p50 目標 1.20 s を超えている**（下記） |
...
#### ★ 85% 規則は Phase 2 ではなく、**もう破れている**〔2026-08-17〕

**規則: 区間合計は p50 目標の 85% 以下**（= 1.02 s）。`vad_ms` を実際の値に直した結果:

| | 区間合計 | p50 目標比 | 判定 |
|---|---|---|---|
| 当初の表（測る前） | 1.02 s | 85% | ちょうど上限 |
| **Phase 1（実測反映後）** | **1.27 s** | **106%** | **予備枠どころか目標を超えている** |
| Phase 2（記憶検索 0.05 s） | 1.32 s | 110% | |

**ただし完了条件は p95 < 2.0 s であり、これは満たしている**（ウォーム p95 1.63 s）。
超えているのは **p50 目標 1.20 s** で、これは実測前に置いた Provisional な値である。
...
#### 残った差は、実測ではなく予算表の算術である

**縮めても 106% は動かない。** この表の「区間合計」は**予算の合計**であり、
実測を速くしても変わらないからである。

```
0.43(vad) + 0.22 + 0 + 0.03 + 0.28 + 0.07 + 0.20 + 0.04 = 1.27 s  >  p50 目標 1.20 s
```

**`vad_ms` = 0.43 s は動かせない**（ターンテイキングの方針。上記）。**残り全部の予算が
0.59 s 以内**でなければ 85% 規則を満たさないが、現在の合計は 0.84 s である。
つまり **p50 目標 1.20 s は、0.43 s の VAD と両立しない。**

| 案 | 得るもの | 失うもの |
|---|---|---|
| **p50 目標を 1.5 s に置き直す**（区間合計 1.27 / 0.85 ≒ 1.49） | 警告灯が機能を取り戻す | 目標が緩む |
| `llm_first_token` をさらに縮める（実測 421 / 予算 280） | 本質的な改善 | **モデルを小さくする以外に手が無い**（9.7B Q4 でこれが底） |
| `tts_first_audio` を縮める（実測 418 / 予算 200） | 同上 | エンジン側の固定費 |

**残りはユーザーの判断待ち**（何を約束するかの決定であり、実装の選択ではない）。
**常に赤い SLO は警告灯として死ぬ。** `unaccounted_ms` の計器が壊れていたときと同じ失敗なので、
**Phase 2 に持ち越さずに決める。** → [../roadmap.md](../roadmap.md) 未確定事項 8b
...
**現在この規則は満たしていない**（1.27 / 1.20 = 106%）。上記のとおり、直し方は未決である。
