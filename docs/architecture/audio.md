...
**どう直すかは未決。** 選択肢は3つあり、どれも Phase 1 の完了条件には影響しない。

| 案 | 得るもの | 失うもの |
|---|---|---|
| p50 目標を実測に合わせる（1.5 s 前後） | 警告灯として機能し続ける | 目標が緩む |
| `llm_first_token`（実測 537 ms / 予算 280 ms）を縮める | 本質的な改善 | モデルを小さくすると会話品質が落ちる |
| `tts_first_audio`（実測 418 ms / 予算 200 ms）を縮める | 同上 | エンジン側の固定費なので手が届きにくい |
