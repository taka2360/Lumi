...
### リングにロックを置かない代わりに、カウンタの書き手を1つに固定する

コールバックはロックを取れないので、リングの整合性は**「各カウンタの書き手が常に1つ」**という
規約だけで保つ。書き込み側は書き込み位置、読み出し側は読み出し位置しか進めない。
`clear()` は読み出し位置を書かず、**破棄マークを publish する**（規約と根拠は `audio/ring.py`）。

**`x += n` は load-add-store であり不可分ではない。** 両側が同じカウンタを進めると更新が失われ、
**失われても例外にはならない** —— 残り時間ずっと「読める量」が過大なままになり、
上書き済みのサンプルを読み続ける。**黙って劣化する典型なので、構造で塞ぐ。**
サンプル本体は同期していないので、コピー中に追い越されたウィンドウは**コピー後に検出して捨てる**
（2つの瞬間が混ざったウィンドウを推論に渡すより、ウィンドウが1つ無い方が良い）。
...
| `min_silence_duration_ms` | 400 | 区間確定。**この 400 ms の待ちに STT を重ねる** → §7 の投機 STT |
...
| | 目標 |
|---|---|
| p50 | **< 1.5 s**〔2026-08-18 改定。下記〕 |
| p95 | < 2.0 s |
| p99 | < 3.0 s |
| Hard failure | > 5.0 s（「考え中」の表現を出す） |

> **p50 は 1.2 s から 1.5 s に置き直した。** 実測前に置いた値が、`vad_ms` の実測（0.43 s、
> ターンテイキングの方針であって最適化対象ではない）と両立しなかったため。
> **p95 / p99 / 完了条件は変えていない。** → 下記「85% 規則」

### 区間別 p50 目標 — クリティカルパスの表

**この表はクリティカルパスである。** 重なる区間は「重なる」と書き、
クリティカルパスには寄与分だけを計上する〔2026-08-22 改定。[ADR-039](../decisions/ADR-039-speculative-stt.md)〕。
...
| 第1文 TTS 生成 | `tts_first_audio_ms` | 0.20 s | 0.20 s | AivisSpeech への HTTP 往復を含む。**GPU 0.44 s / CPU 0.90 s**（実測。いずれも未達） |
| 再生開始（バッファ + IPC） | `playback_ms` | 0.04 s | 0.04 s | |
| **クリティカルパス合計** | | | **1.10 s** | 記憶検索を含む Phase 2 の値 |
| **予備** | | | — | EventBus の永続化・GC・スケジューリング・計測外の処理 |
| **合計** | | | **1.10 s** | p50 目標 1.50 s の **73%**（下記） |
...
| | 規則 | |
|---|---|---|
| 開始 | VAD が**無音の開始**を検出した時点（**`SILENCE_STARTED`**。`min_silence_duration_ms` の経過を待たない） | スナップショットは**不変のコピー**（**非破壊**。バッファをドレインしない）。**世代 ID**（ターン ID + バッファ世代）を付ける |
| **採用** | 発話終端が確定したら、**確定した世代と結果の世代を原子的に照合し、一致したものだけ採用する** | **不一致・世代不明・タイムアウトは採用しない**（fail-closed）。現在のスナップショットで実行し直す |
| **破棄** | **無音が途切れて発話が続いたら世代を進める。** 古い世代の結果は照合で落ちる | **異常ではない。**`stt.speculation_discarded` として数えるだけ |
| **有界性** | **single-flight**（同時に走る STT は1つ）。走行中に世代が進んだら**保留スロットを上書き**する（latest-wins） | **STT の推論はキャンセルできない**（`asyncio.to_thread`）。有界性は起動を絞って作る |
| **上限** | `max_speculations_per_turn`（既定 3）を超えたらそのターンは投機をやめ、**終端確定後に1回だけ実行する** | `stt.speculation_capped`。**上限に当たったターンは Phase 1 と同じ速さに戻る** |
| **経路** | **投機・上限超過後のフォールバック・終端確定後の実行を、すべて同じ single-flight の所有者に投入する** | **`SPEECH_ENDED` から STT を直接起動しない**（分けると2本目が走る） |
| **層** | audio callback でも VAD 専用スレッドでもなく、**別のワーカー**（§2 の3層分離） | **ミュート判定は投機 STT を待たない** |

**「先に終わった結果を使う」にしてはならない。** 速さで勝敗を決めると、
**ユーザーが言っていない文（1文目だけの投機結果）で Lumi が返事をする。**
根拠と設計の全体は [ADR-039](../decisions/ADR-039-speculative-stt.md)。

**CPU 構成で隠れきらないことは、上記「重なる区間の寄与」の表が扱う。**

#### ★ `vad_ms` は実装のコストではなく、ターンテイキングの方針である〔2026-08-17 決着〕

**予算 0.18 秒と `min_silence_duration_ms` = 400 ms は、同じ待ち時間を2箇所に別々の数値で
書いていた。** 実測 429 ms はパラメータどおりであり、**間違っていたのは予算の側**である。

**パラメータを下げる案は捨てた。** 下げれば -200 ms 稼げるが、**文中の自然な間で区間が切れる。**

> **〔2026-08-17 実測〕** 2文（3.14 秒）の音声を capture → VAD 経路に流したところ、
> **現行の 400 ms でも既に2区間に分割された**（1文目 1.25 秒で SPEECH_ENDED）。
> STT は1区間目を正しく認識したが、**Lumi は1文目だけを聞いて返事を始める。**
> 200 ms に下げれば、これは読点程度の間でも起きる。

つまり 400 ms は**短すぎることはあっても長すぎない。** この 0.43 秒は
「**ユーザーが喋り終わったと判断するまで待つ**」という方針の値であり、
**最適化で消える種類の遅延ではない。**

**SSoT: この区間の唯一の定義は §5 の `min_silence_duration_ms` である。**
表に独立した数値を書くと、また片方だけが動く。
...
#### ★ p50 目標を 1.2 s → 1.5 s に置き直した〔2026-08-18 決着。未確定事項 8b〕

**規則: 区間合計は p50 目標の 85% 以下**（当初 = 1.02 s）。`vad_ms` を実際の値に直した結果:

| | 区間合計 | 旧 p50 (1.20 s) 比 | **新 p50 (1.50 s) 比** |
|---|---|---|---|
| 当初の表（測る前） | 1.02 s | 85% | 68% |
| **Phase 1（実測反映後）** | **1.27 s** | 106% ✗ | **85%** ✓ |
| Phase 2（記憶検索 0.05 s を直列で足す） | 1.32 s | 110% ✗ | 88% ✗ |
| **Phase 2（投機 STT で `stt_ms` を重ねる）** | **1.10 s** | 92% ✗ | **73%** ✓〔[ADR-039](../decisions/ADR-039-speculative-stt.md)〕 |

**完了条件は p95 < 2.0 s であり、これは変えていない**（ウォーム p95 1.63 s で達成済み）。
超えていたのは **p50 目標 1.20 s** で、これは実測前に置いた Provisional な値である。

**方針: 目標を緩める前に、まず実測を縮める**〔2026-08-18 決定〕。
以下は `llm_first_token` に対して実施済み。

| やったこと | `llm_first_token` |
|---|---|
| （それまで） | 537〜714 ms |
| **HTTP クライアントの使い回し**（毎ターン `AsyncClient` を作り直していた） | **421 ms**（-148 ms） |

**これは最適化ではなくバグ修正だった。** 同じ誤りを TTS 側では既に直しており（改善経過 ④）、
**LLM 側だけが残っていた。** 併せて、起動直後の1ターン目が 7.5 秒かかっていた原因
（3つの Provider が全部「ロードしたふり」をしていた）も解消した。
→ [../measurements/phase1.md](../measurements/phase1.md)「起動直後の1ターン目が 7.5 秒だった」

#### 残った差は、実測ではなく予算表の算術だった

**縮めても 106% は動かなかった。** この表の「区間合計」は**予算の合計**であり、
実測を速くしても変わらないからである。

```
0.43(vad) + 0.22 + 0 + 0.03 + 0.28 + 0.07 + 0.20 + 0.04 = 1.27 s  >  旧 p50 目標 1.20 s
```

**`vad_ms` = 0.43 s は動かせない**（ターンテイキングの方針。上記）。**残り全部の予算が
0.59 s 以内**でなければ 85% 規則を満たさないが、合計は 0.84 s である。
つまり **p50 目標 1.20 s は、0.43 s の VAD と両立しなかった。**

**決定: p50 目標を 1.5 s に置き直す**（1.27 / 0.85 ≒ 1.49）。

| 採らなかった案 | 採らなかった理由 |
|---|---|
| `llm_first_token` をさらに縮める（実測 421 / 予算 280） | **モデルを小さくする以外に手が無い**（9.7B Q4 でこれが底）。会話品質を SLO のために落とす順序が逆 |
| `tts_first_audio` を縮める（実測 418 / 予算 200） | エンジン側の固定費で手が届かない |

**「目標が緩む」ことは受け入れる。** 1.2 s は測る前に置いた数値であり、**実測が目標に届かない
のではなく、目標が実測できる構造と矛盾していた。** 常に赤い SLO は警告灯として死ぬので、
**達成できない数値を掲げ続けるほうが失う**（`unaccounted_ms` の計器が壊れていたときと同じ失敗）。

**縮める努力を先にやってから緩めた。** `llm_first_token` は 537〜714 → 421 ms、
起動直後の1ターン目は 7.5 s → 約 1.7 s。**測らずに目標だけ動かしてはいない。**

> **区間別の予算は据え置く。** `llm_first_token` 0.28 s も `tts_first_audio` 0.20 s も
> 実測（421 / 418 ms）に届いていないが、**予算まで実測に合わせると「全部達成」になり、
> どこが重いかが表から消える。** 区間別の予算は現状の記述ではなく、**目標である。**
...
**規則: クリティカルパス合計は p50 目標の 85% 以下に収める。** 区間を追加するときはこの制約を先に確認する。

**この規則の出番は既に一度あった。** Phase 2 の記憶検索 0.05 s を直列で足すと 88% になり、規則を破る。
そこで**目標を動かさず、区間の並べ方を変えた**——STT を VAD の無音待ちに重ねて、
クリティカルパスを 1.10 s / **73%** にした（[ADR-039](../decisions/ADR-039-speculative-stt.md)）。

**予備枠が 15% から 27% に増えたことが、この改定の本当の利得である。** Phase 3 以降も
クリティカルパスに載る候補は増え続ける。85% に張り付いたままでは、何かを足すたびに同じ議論をやり直すことになる。
...
{"event": "turn_latency", "correlation_id": "...",
 ...
 "stt_wait_ms": 0, "stt_discarded_ms": 0, "stt_discarded": 0,
...
**区間は連続しているか、重なると明示されているかのどちらかでなければならない**〔2026-08-22 改定〕。
...
**投機は1ターンに複数回走りうるが、採用されるのは必ず1回である。**

| キー | 何を入れるか |
|---|---|
| **`stt_ms`** | **採用された実行 1 回分だけ。**〔始端〕実行を要求した時刻。〔終端〕結果が採用可能になった時刻。**ワーカーの空き待ちを含む** |
| `stt_wait_ms` | `stt_ms` の内訳。**ワーカーが空くのを待った時間**（先行する実行がキャンセルできず走り続けた分） |
| **`stt_overlap_ms`** | **採用された実行と `vad_ms` の重なりだけ。** 採用は1回なので**区間のマージは起きない** |
| `stt_discarded_ms` | 破棄された実行の所要時間の合計。**`stt_ms` に足さない。`critical_path_ms` にも足さない** |

**破棄された実行を `critical_path_ms` に足さないのは、無視してよいからではない。**
クリティカルパス上にある分は既に `stt_wait_ms` として `stt_ms` に入っており、
**両方足すと二重計上になる**からである。

> **`stt_ms` の始端を「実行を要求した時刻」にしたのは、この遅延を消さないためである。**
> 「推論が実際に始まった時刻」を始端にすると、キャンセルできない先行実行を待たされた時間が
> **誰の区間でもなくなり、`unaccounted_ms` に化ける。**
...
#### 書き出しは撤去した〔2026-08-22 / Phase 2a〕

Phase 1 では確定した発話区間を `<data_dir>/debug/stt/` に **16 kHz mono WAV + 認識結果の .txt**
で書き出していた（`lumi/audio/dump.py`。ソースから実行したときは既定で有効）。
**[../contracts/privacy.md](../contracts/privacy.md) §6 がこれを禁じたので、経路ごと消した。**

> 生の音声波形は永続化しない。**デバッグ用の録音経路も作らない。**
> 「開発時だけ」の抜け道は、そのまま出荷される。

実際、この実装は `LUMI_DEBUG_STT_DUMP=1` で**配布物でも有効にできた**。契約が想定していた形そのものである。

代わりに残すのは、**音声を書き出さずに取れる指標**である。区間ごとに秒数・ピーク・RMS を
`reactive.segment` として構造化ログに出す（**認識結果のテキストは載せない**。
Episode は暗号化して保存するのに、同じ内容が平文のログに残っては意味がない）。

**これは等価な代替ではない。** 上の折り返しはピークの張り付きとして現れるが、語頭欠落は現れない。
**聞けば1秒で分かるものを、指標で追うことになる**——それが契約の対価である。
再び録音経路が必要になったら、[ADR-038](../decisions/ADR-038-privacy-and-data-retention.md) を
修正する ADR を書いてからにする。**黙って戻さない。**
...
| 7c | **上限文字数がチャンクの切れ方に依存しない**（1チャンクの中に終端が上限より後ろで来ても、セグメントは上限を超えない） |
| 8 | interrupt 時に生成中の TTS タスクが破棄される |
| 8b | **中断で再生キューが捨てられる**（古い音が後から再開しない） |
| 8c | **1文の TTS 失敗が発話全体を落とさず、失敗として数えられる**（黙って飛ばさない） |
| 8d | **Provider が契約外の例外を投げても発話が固まらない**（文のスロットは必ず解決される。未解決のまま残ると再生ループごとターンが止まる） |
...
| 12b | **リングの各カウンタの書き手が1つだけ**（静的検査）。コピー中に書き手に追い越されたウィンドウは返さない（§3） |
...
| 19 | **VAD スレッドが例外で終わってもミュートを握ったままにしない**（ログに残す。フラグを戻すのはこのスレッドだけなので、握ったまま死ぬと恒久的な無音になる） |
| 20 | **再生リングと reference リングが、チャンネル数によらず表記どおりの秒数を保持する**（どちらも interleaved サンプルを持つ。reference の長さは Phase 2 の AEC 要件） |
| **21** | **`SILENCE_STARTED` が無音の開始フレームで1回だけ出る**（毎フレーム出ると投機が再起動し続ける） |
| **21b** | **スナップショットが非破壊である**（`take()` を使うと、確定した区間が投機に食われて末尾だけになる） |
| **21c** | **発話が再開したら世代が進み、古い世代の投機結果が採用されない**（速さで勝敗を決めない） |
| **21d** | **同時に走る STT が常に1つ**（世代を N 回進めても。保留は最新の1つだけ） |
| **21e** | **上限に達したターンで、保留が起動されない**（判定・破棄・解放が同じ遷移であること） |
| **21f** | **`stt_overlap_ms` が実測され、隠れきらない分がクリティカルパスに残る**（定数 0 で埋めない） |
