...
| 区間 | ログのキー | 目標 | 備考 |
|---|---|---|---|
| VAD 発話終端の確定 | `vad_ms` | **§5 の `min_silence_duration_ms` + フレーム境界**（現在 0.43 s） | **ここに独立した数値を書かない**（下記） |
...
| **区間合計** | | **1.32 s** | Phase 1 は記憶検索が無いので **1.27 s** |
| **予備** | | — | EventBus の永続化・GC・スケジューリング・計測外の処理 |
| **合計** | | **1.27 s** | p50 目標 1.50 s の **85%**（下記） |
...
| Phase 2（記憶検索 0.05 s） | 1.32 s | 110% ✗ | 88% ✗（Phase 2 で再検討） |
...
**規則: 区間合計は p50 目標の 85% 以下に収める。** 区間を追加するときはこの制約を先に確認する。

**この規則は現在ちょうど満たしている**（1.27 / 1.50 = 85%）。
**Phase 2 で記憶検索 0.05 s を足すと 88% になり、また破れる。** そのときは
「区間を追加する前にこの制約を確認する」の出番であって、目標を再び動かす理由にはしない。
