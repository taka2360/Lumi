...
| | 何を保持するか | 長さ |
|---|---|---|
| **プリロール** | 閾値を超える**前**のフレーム | `speech_pad_ms`（80 ms） |
...
`LUMI_DEBUG_STT_DUMP=1` で、確定した発話区間を `<data_dir>/debug/stt/` に
**WAV + 認識結果の .txt** で書き出す（`lumi/audio/dump.py`）。既定は無効。
上の折り返しは Core が出すどの指標にも現れず、**聞けば1秒で分かった。**
