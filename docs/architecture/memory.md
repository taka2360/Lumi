...
- ★ **生成を始める前に、その都度 revoke を確認する。** Provider は `/api/chat` を投げてから
  トークンを見るので、確認がパスの入口だけだと **revoke 済みの2回目の背景生成が
  ユーザーのターンと競合する**。revoke は Job が乗っている任意の await に着地するので、
  Episode ループ入口の確認の後にある `utterances_from()` と `recent()` がその窓になる。
  確認は `_ask` の直前に置く——**縮めた再試行も、同じバッチでの再サンプルも、
  等しく「2回目の背景生成」である**

##### 生成の終わり方と、watermark をどうするか〔2026-09-04 / [ADR-049](../decisions/ADR-049-truncated-extraction-and-watermark.md)〕
...
| 打ち切り後・次の試行の前の revoke | 引かない | **動かさない** | 書かない | true | — |
...
- **`length` だけは「次回やり直す」では終わらない。** トークン上限による切断は、
  **同じ発話列を同じ上限で読む限り次回も同じ場所で起きる**。`unreflected()` は古い順に返すので、
  ...
  したがって **その場でバッチを半分にして引き直す**。縮めて読み残した分は次のパスが読むので、
  **縮めることで失うものは無い**
...
- ★ **1回のパスが失敗しても scheduler のループは死なない。** `ReflectionScheduler` は起動時に
  ...
  Lumi は喋り続けるのに二度と覚えず、画面にはそれを示すものが何も出ない。
  繰り返しは構造上安全である（watermark は書き込みの後にしか動かない）
