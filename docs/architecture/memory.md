...
- **ただし `Finish(reason="length")` だけは「次回やり直す」では終わらない。**
  トークン上限による切断は、**同じ発話列を同じ上限で読む限り次回も必ず同じ場所で起きる**。
  `unreflected()` は古い順に返すので、そのままにすると**その Episode 以降が永久に詰まり**、
  アイドルのたびに推論を無駄に焼く。したがって **その場でバッチを半分にして引き直す**。
