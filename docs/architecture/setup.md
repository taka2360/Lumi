...
|---|---|---|---|
| Lumi が取得するか | Ollama本体: **しない**（検出のみ）<br>許可モデル: **明示同意後にローカル `/api/pull` を依頼** | **する**（同意に基づく） | **する**（同意に基づく） |
| 理由 | Ollama本体のインストーラ導入判断は代行しない。モデル取得は対象・サイズを示して同意を取る | サイズが R1 に直撃する（数百 MB〜） | [ADR-019](../decisions/ADR-019-tts-engine-distribution.md) |
| 未導入のとき | Ollama未導入なら公式サイトを案内。モデル未取得なら名前・サイズを示して取得を明示確認 | 「STT 未セットアップ」＋ 取得の提案 | 「TTS 未セットアップ」 |
...
「別のモデルを選ぶ」では `GET /api/tags` で取得したローカルモデルも名前・サイズ付きで表示し、
既存モデルは `select` として追加取得なしで選べる。
同意後はローカル `POST /api/pull` のNDJSON進捗を表示し、成功後にモデル確認をやり直す。
3つの推論要素が
すべて `ready` なら、そのままセットアップを抜けて会話を開始する。Lumi の再起動は要求しない。

**ポーリングの生存期間は画面に一致させる。** `not_configured`、
`detected × starting`、または `detected × stopped` の画面では Stage がタイマーを動かし、
それ以外になったらタイマーを止める。Core は要求を直列化し、
...
| 36 | **不足が0件なら「まとめて取得しますか」を出さない**〔2g〕 |
| 37 | **「まとめて取得する」に答えたら component ごとに聞き直さない。「今は取得しない」なら何も取得せず、個別にも聞かない**〔2g〕 |
| 38 | **一覧の合計は Core が出した数**であり、Stage が一覧から計算し直さない（読めない行を落としても合計は変わらない）〔2g〕 |
| 39 | **Ollama 本体は一覧に入らない**（Lumi が取得しないものを取得ボタンの下に並べない）〔2g / ADR-023〕 |
| 40 | **埋め込みモデルが無い / 失敗しても `ready` に到達する。取得中だけ `installing` になる**〔2g / ADR-041〕 |
| 41 | **埋め込みモデルのステータス行は `bad` にならない**（他の行は起動できない理由であり、これは検索の質が落ちる理由）〔2g〕 |


---

## 取得の同意 UI〔2026-08-23 / 2g〕

### まとめて聞いてから、個別に聞く

**最初に1問だけ「必要なものをまとめて取得しますか（合計 n GB）」を出す。**

| 答え | 何が起きるか |
|---|---|
| まとめて取得する | 不足しているものを順に取得する。**component ごとに聞き直さない** |
| 個別に選ぶ | 従来どおり component ごとに聞く |
| 今は取得しない | **何も取得せず、component ごとにも聞かない** |

理由は**合計が見えないこと**。4回続けて別々の数字を見せられると、
**実際に決め手になる数字（全部でいくらか）だけが最後まで出てこない。**
逆に「まとめて取得する」と答えた人に component ごとに聞き直すのは、
**同じ質問を4回することであり、同意ダイアログを読まずに閉じる習慣**を作る。

- **不足が0件なら聞かない。** 0バイトの同意ダイアログは、同意ダイアログを軽く扱わせる
- 一覧には**取得先の名前と各サイズ**を出す。**合計は Core が出した数**であり、
  一覧の合計を Stage が計算し直さない（読めない行を落としたとき、
  **落とした分だけ小さい数を見せることになる**）
- **Ollama 本体は一覧に入らない。** Lumi はそれを取得しない（[ADR-023](../decisions/ADR-023-llm-runtime-and-model-acquisition.md)）

### 埋め込みモデルだけは起動をブロックしない

**「3つ揃うまでキャラクターを出さない」（[ADR-034](../decisions/ADR-034-gate-startup-on-complete-setup.md)）の3つに、埋め込みモデルは入らない。**

| | 無いとどうなるか | `boot` |
|---|---|---|
| TTS / STT / LLM | **会話が成立しない** | `blocked` |
| 埋め込みモデル（[ADR-041](../decisions/ADR-041-embedding-model-harrier-oss.md)） | 記憶の意味検索が使えない（語句と新しさで動く） | **`ready` に到達する** |

- 質問は**最後**に出す。断っても Lumi は普通に使えることを、その場で書く
- **`INSTALLING` の間だけは `boot` が `installing` になる**。ブロックはしないが、
  進捗バーの前にキャラクターを立たせると「いま何が起きているか」の答えが2つになる
- ステータス一覧では**最後の行**で、**`bad` にしない**。
  他の行は「起動できない理由」であり、これは「検索の質が落ちる理由」である
