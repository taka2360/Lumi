...
- **発見しやすさは未決。** 初回のみの操作ヒントや、セットアップ完了後の操作説明を後から
...
| 操作 | 経路 | 実装 |
|---|---|---|
| **移動** | `shell.*` | キャラクターの上で `alt` + 左ドラッグ、または中ドラッグ → OS のウィンドウ移動に委ねる。起動・セットアップパネルの上では素の左ドラッグでも動く |
...
| **右クリック** | — | アプリ操作パレット（§1）。キャラクターの上でのみ |

**どれも、その時点で画面に出ている操作面の上でだけ効く。** キャラクターをまだ出せない
`setup` / `installing` / `starting` / `blocked` では、パネルが一時的な操作面になる。

**左ボタンを予約するのはキャラクターの上だけ。** パネルには予約すべきインタラクションが
無く、そこで移動まで修飾キーを要求すると、キャラクターに会う前のユーザーが
ウィンドウを動かせなくなる。パネルは従来どおりホバーで操作列を出す。
