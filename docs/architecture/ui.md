...
言語は起動時に OS / WebView の優先言語から解決し、`ja` で始まる言語タグだけを日本語、
それ以外を英語にする。言語タグが取得できない場合も英語へフォールバックする。
...
| 領域 | ロケールの取得元 | 実装上の規則 |
|---|---|---|
| Stage / クレジット | `navigator.languages` → `navigator.language` | 翻訳カタログを単一定義にし、`document.documentElement.lang` も同じ値にする |
| Shell のトレイ / ネイティブウィンドウ | OS のユーザー既定ロケール（取得不能時は `LANG` 系環境変数） | Stage と同じ `ja` / `en` 解決規則を純粋関数でテストする |
