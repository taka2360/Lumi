...
| 再接続 | **`seq` はリセットされうる。** **Core が採番する接続世代（epoch）と組で `(epoch, seq)` を辞書順に比べる**（下記） |
| 捨てたとき | **ログに出す。** 恒常的に出るなら、周期か経路がおかしい |

##### ★ 接続世代（epoch）は Core が採番する

**`seq` のリセットを送出元に判断させない。** 再接続した Shell は `seq` を 1 から振り直すが、
**そのとき古い接続のハンドラがまだ走っていることがある**——
接続の差し替えで実行中の request タスクが止まるわけではない
（`core/lumi/transport/server.py` がそれを行うのは停止時だけである）。

| どちらに倒しても壊れる | |
|---|---|
| 受理済み `seq` をリセットする | **古いハンドラが新しい世代を上書きできる** |
| リセットしない | **新しい接続の観測が、古い高水位に追いつくまで全部拒否される** |

**接続ごとに Core が単調増加の `epoch` を採番し、`(epoch, seq)` を辞書順に比べる。**

```text
受理する ⇔ (epoch, seq) > 最後に受理した (epoch, seq)
```

**これで `seq` のリセットが問題にならない。** 古い接続のハンドラは `epoch` が小さいので
**`seq` が幾つであっても負け**、新しい接続は `epoch` が大きいので **`seq` が 1 でも通る。**

| | |
|---|---|
| 誰が付けるか | **Core**（接続確立時）。**payload に入れさせない**——送出元が付けた値は B8 の対象である |
| どう届くか | **inbound ハンドラが接続の文脈を受け取る必要がある。** 現在 `InboundHandler` は **payload しか受け取らない**ので、**3a で接続 identity（role / peer / epoch）を渡す形に変える**——`Signal.source_id` を接続から決める要件（[../contracts/security-boundaries.md](../contracts/security-boundaries.md) B8）と**同じ変更である** |
| どこで比べるか | **下記の排他区間の中**。`seq` と同じ扱い |
...
| **あとで止める** | **設定から取り消せる。取り消したらその場でポーリングスレッドを止める**（下記） |
| 保存先 | `world:*` の DomainEvent は既定 30 日（[../contracts/privacy.md](../contracts/privacy.md) §2 の行 5）。**新しい保存先は作らない** |

**★ 取り消せない同意は同意ではない。**
一度許可したら設定ファイルを手で編集するまで止められない、という状態にしない。

| | |
|---|---|
| どこで | **設定パネル**（[../decisions/ADR-042-panel-windows-and-panel-role.md](../decisions/ADR-042-panel-windows-and-panel-role.md) の独立ウィンドウ）。マイクのミュートと同じ「今それが動いているか」を扱う場所に並べる |
| 何が起きるか | **その場でポーリングスレッドを止める。** 次回起動時ではない——**止めたのに観測が続くのは、止まっていないのと同じ** |
| 残った facet | **TTL で自然に `Unknown` になる。即座に消さない**（消すと「なぜ急に何も知らなくなったか」を説明できない）。**TTL のあいだは消えないことを画面に書く** |
| 再度 許可するとき | **同意をやり直す**（前回の許可を再利用しない）。観測 key が増えていれば差分も見せる |
| 自律への影響 | 止めた時点から `AutonomyGate` は `Unknown` を通さない側に倒れる。**設定1つで自律発話が止まる**ことを画面に書く |
...
| 15b | **設定で取り消すと、その場でポーリングスレッドが止まる**（次回起動時ではない） |
| 15c | **取り消し後、残った facet は TTL で `Unknown` になる**（即座に消えるのではない） |
| 15d | **取り消してから再度許可すると、同意をやり直す**（前回の許可を再利用しない） |
...
| 18h | **古い接続のハンドラが、新しい接続の観測を上書きしない**（旧 epoch の `seq` 99 と新 epoch の `seq` 1 を競わせる。**`seq` だけで比べる実装で落ちること**） |
| 18i | **再接続後の `seq` 1 が拒否されない**（epoch が上がっているので通る） |
