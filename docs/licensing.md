...
| 2 | **既定音声モデルの個別ライセンス**。〔2026-08-15 実測〕Engine は同梱ではなく、**初回起動時に AivisHub から既定モデルを取得する**（BERT モデルは HuggingFace から）。したがって適用されるのは §4.4 の ACML 等であり、取得行為には Aivis Project 利用規約が効く（§3）。**取得するのはユーザーの PC 上のエンジンであって Lumi ではない** | 既定音声として使う場合 / クレジット表示 | Phase 0-1 |
