...
#### 2g — ユーザーが見て直せること〔2026-08-23。**埋め込みモデルの取得 UI だけ残っている**〕

- [x] **記憶の閲覧・編集・削除 UI** — 独立ウィンドウ `memory`
  （[ADR-042](decisions/ADR-042-panel-windows-and-panel-role.md)）。
  **編集は supersede**（上書きしない）で、書いた本人の文なので `user_confirmed` になる
- [x] **「全部消して」**（`retention.erase_everything()`。**先に `erase_preview` が
  privacy.md §2 の行ごとの件数を返す。0 件の行も返す**）
- [x] **エクスポート**（平文 JSON。**書き出す前に平文であることを画面に出す**）
- [x] アンインストーラの「覚えたことも削除する」（既定オフ。`shell/src-tauri/nsis/uninstall.nsh`）
  〔**サイレントアンインストールでは消さない** — 答える人がおらず、沈黙は同意ではない。
  **未実測**: インストーラを実際に走らせた確認はしていない〕
- [x] **マイクが開いていることの常時表示と、即座のミュート**
  〔ホバーではなく常時表示・常時ヒット領域。**ミュートは入力ストリームを閉じる**ので、
  「開いている」と「ミュート」は排他になる〕
- [x] **設定・インスペクタを独立ウィンドウにし、操作列をアイコンにした**（ADR-042）
- [x] 「今の覚えておいて」の明示トリガ（2f から持ち越し）
  〔**待ち時間を短くするのであって、飛ばさない。** 既定 20 秒の静けさを待つ〕
- [ ] **埋め込みモデルの取得 UI**（2e から持ち越し。**唯一の残り**）
  〔取得経路は実装済みで、足りないのは同意 UI と進捗表示。
  セットアップの4つ目の component になるが、**embedding は起動をブロックしない**——
  そこが TTS / STT / LLM と違い、`boot` ゲートの扱いを決める必要がある〕
