...
- [x] **埋め込みモデルの取得 UI**（2e から持ち越し）
  〔4つ目の component `embedding`。**起動をブロックしない**——
  断っても `ready` に到達する。取得中だけ `installing` になる〕
- [x] **「まとめて取得しますか（合計 n GB）」を最初に1問**
  → [architecture/setup.md](architecture/setup.md)
  〔4回別々に聞くと**合計が最後まで出てこない**。答えは
  「まとめて」「個別に選ぶ」「今は取得しない」の3つ〕
